...
$resultado = mysqli_query($con," select f.numero_factura,f.fecha_factura,c.nombres,c.apellido,f.total_venta,u.firstname,u.lastname,u.user_id from facturas f left join  clientesp c on c.id=f.id_cliente left join users u on f.id_vendedor=u.user_id where f.fecha_factura between '$fi 0:00:00' and '$ff 23:59:99' and u.user_id=$cobrador");
$total = 0;

$sql = mysqli_query($con,"select firstname,lastname from users where user_id=$cobrador");
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(10, 8, '', 0);
$pdf->Cell(100, 8, 'Cobrador(a): '.$rows['firstname'].' '.$rows['lastname'], 0);
$pdf->Cell(50, 8, '    Fecha Inicial: '	.$fi.'        Fecha Final: '.$ff, 0);
$pdf->Ln(8);
