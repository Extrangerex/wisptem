...
$sql = mysqli_query($con," select f.condiciones,f.numero_factura,f.fecha_factura,c.nombres,c.apellido,f.total_venta,u.firstname,u.lastname,u.user_id from facturas f left join  clientesp c on c.id=f.id_cliente left join users u on f.id_vendedor=u.user_id where f.fecha_factura like '%$fecha%' and u.user_id=$cobrador");
$total = 0;

$query = mysqli_query($con, "select firstname,lastname from users where user_id=$cobrador");
...
$pdf->SetFont('Arial', '', 9);
$pdf->Cell(160, 6, '', 0);
$pdf->Cell(30, 8, date('Y-m-d H:i:s'), 0);
$pdf->Ln(15);
