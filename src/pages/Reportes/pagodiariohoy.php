...
$pdf->AddPage();
$fecha = date('Y-m-d');
$sql = mysqli_query($con," select f.condiciones,f.numero_factura,f.fecha_factura,c.nombres,c.apellido,f.total_venta,u.firstname,u.lastname,u.user_id from facturas f left join  clientesp c on c.id=f.id_cliente left join users u on f.id_vendedor=u.user_id where f.fecha_factura like '%$fecha%'");
$total = 0;
